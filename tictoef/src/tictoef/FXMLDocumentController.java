    ...
    public void update_board(String message) {
        ...
        if (gl.check_winner(x) == 1) {
            ...
            if (Tictoef.whoX.equals(Tictoef.userName)) {
                who_win.setText(Tictoef.userName);
                score.clear();
                sc+=5;
                 score.setText(sc.toString());

            } else {
                who_win.setText(Tictoef.opponent);
                score.clear();
                sc-=5;
                 score.setText(sc.toString());
            }
        } else if (gl.check_winner(x) == 0) {
            gameEnd.toFront();
        ...
        } else if (gl.check_drwa(x) == true) {
            gameEnd.toFront();
            who_win.setText("game draw ");
        }
        turn.clear();
        Platform.runLater(() -> {
            turn.setText(Tictoef.userName);
        });
    }
...
//      object.array[2][1]=1;
    ...
    void initialize() {
        ...
        Platform.runLater(() -> {turn.setText(turn_who);});
 ...
 turn.clear();
    ...
    private void goBack(ActionEvent event) throws IOException {
        ...
        Parent home_page_parent1 = FXMLLoader.load(getClass().getResource("tablescene.fxml"));
    ...
    @FXML
    void send() {
        String mess = typingField.getText();
        chatArea.appendText(mess + "\n");
        typingField.clear();
        data sms = new data();
        sms.type = "chat";
        sms.message = mess;
        sms.opposit = Tictoef.opponent;
        Gson gson = new Gson();
        ps.println(gson.toJson(sms));
    }

    void update_chat(String message) {
        Platform.runLater(() -> {
